...
int texture( int argc, char **argv) {
    ...
    switch (contenuCase.contenu) {
		...
		case 2:
			switch (contenuCase.mstr.type) {
				...
				case 3:
					...
					break;
				case 4 :
					if (bossPresent) {  // Gestion des morceaux du boss
					switch (contenuCase.mstr.morceau) {
						case 0:
							image = SDL_LoadBMP("src/image/bossHautGauche.bmp");
							break;
						case 1:
							image = SDL_LoadBMP("src/image/bossHautMilieu.bmp");
							break;
						case 2:
							image = SDL_LoadBMP("src/image/bossHautDroite.bmp");
							break;
						case 3:
							image = SDL_LoadBMP("src/image/bossBasGauche.bmp");
							break;
						case 4:
							image = SDL_LoadBMP("src/image/bossBasMilieu.bmp");
							break;
						case 5:
							image = SDL_LoadBMP("src/image/bossBasDroite.bmp");
							break;
						default:
							SDL_DestroyRenderer(renderer);
							SDL_DestroyWindow(window);
							SDL_ExitWithError("Morceau du boss invalide");
							break;
                		}
					} else {
						image = SDL_LoadBMP("src/image/sol.bmp");  // Sol par défaut si le boss n'est pas apparu
					}
            		if (image == NULL) {
						SDL_DestroyRenderer(renderer);
						SDL_DestroyWindow(window);
						SDL_ExitWithError("Impossible de charger l'image du boss ou du sol");
					}
           			break;
				default:
					SDL_DestroyRenderer(renderer);
		...
		case 3:
			...
			break;
		// case avec porte du boss
        case 4:
            image = SDL_LoadBMP("src/image/porteBoss.bmp");
            if(image == NULL){
		        SDL_DestroyRenderer(renderer);
		        SDL_DestroyWindow(window);
